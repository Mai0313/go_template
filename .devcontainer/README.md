...
- **Features**:
    ...
    - `go`: the Go toolchain plus `gopls`, `staticcheck`, `golangci-lint`, `dlv`, `revive` and the rest of the tools the VS Code Go extension expects.
- **devcontainer.json**: extension recommendations (Go, Docker, GitLens, YAML, TOML) and a zsh terminal profile.
- **updateContentCommand**: runs `go mod download`, so modules are cached when the container opens.

`make lint-deadcode` installs `staticcheck` and `deadcode` on demand if they are missing, so no tool needs to be baked into the image for it.
...
3. Then `make build`, `make test`, `go mod tidy` as usual.

## Customization

- **Pin a Go version**: pass `{"version": "1.XX"}` to the `go` feature. Keep it consistent with the `go` directive in `go.mod`.
