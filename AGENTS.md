...
| Containers and development environment | Update maintainer labels, image name labels, binary `COPY`, `chmod`, and `CMD` in `docker/Dockerfile`. `.devcontainer/` has no Dockerfile: it is a stock image plus Dev Container Features, carries no project identity, and normally needs no rename.              |
